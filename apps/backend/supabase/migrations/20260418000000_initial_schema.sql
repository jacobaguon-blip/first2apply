...
-- Stripe DB wrappers removed for household self-host (no paid subscriptions).
-- Original block created the stripe_fdw extension, stripe_wrapper FDW,
-- stripe_server, and stripe.customers / stripe.subscriptions foreign tables.
-- The stripe_customer_id / stripe_subscription_id columns on public.profiles
-- are retained (harmless nullable text) so no downstream code breaks.
